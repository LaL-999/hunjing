<script setup lang="ts">
/**
 * Icon — 全站唯一 SVG 图标出口(INS-A9,2026-05-27 末³).
 *
 * 设计来源:Lucide(细线 SVG line icon)
 * 规范:viewBox 24×24 / stroke=currentColor / stroke-width=1.5 / fill=none
 *
 * 用法:<Icon name="play" :size="14" />
 *
 * 禁止:
 *   - 任何 UI 控件直接写 emoji 字面量(▶ ✓ ✗ 📈 📉 ⚠️ 🔍 💡 等)
 *   - 装饰区块标题前缀 emoji 也走本组件
 *   - 新加 icon 直接在本文件加 v-else-if 分支,不另起组件
 *
 * 协作偏好溯源:feedback_collab_style.md 2.7(2026-05-23 立规,2026-05-27 末³ 复发治本)
 */

withDefaults(
  defineProps<{
    name: string;
    size?: number | string;
  }>(),
  { size: 16 },
);
</script>

<template>
  <svg
    :width="size"
    :height="size"
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
    stroke-width="1.5"
    stroke-linecap="round"
    stroke-linejoin="round"
    class="icon"
    aria-hidden="true"
  >
    <!-- 触发 / 状态 -->
    <template v-if="name === 'play'">
      <polygon points="5 3 19 12 5 21 5 3" />
    </template>
    <template v-else-if="name === 'check'">
      <polyline points="20 6 9 17 4 12" />
    </template>
    <template v-else-if="name === 'x'">
      <line x1="18" y1="6" x2="6" y2="18" />
      <line x1="6" y1="6" x2="18" y2="18" />
    </template>
    <template v-else-if="name === 'refresh'">
      <path d="M21 12a9 9 0 0 0-15-6.7L3 8" />
      <path d="M3 3v5h5" />
      <path d="M3 12a9 9 0 0 0 15 6.7L21 16" />
      <path d="M21 21v-5h-5" />
    </template>

    <!-- 趋势 / 数据可视化 -->
    <template v-else-if="name === 'trending-up'">
      <polyline points="22 7 13.5 15.5 8.5 10.5 2 17" />
      <polyline points="16 7 22 7 22 13" />
    </template>
    <template v-else-if="name === 'alert-triangle'">
      <path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z" />
      <line x1="12" y1="9" x2="12" y2="13" />
      <line x1="12" y1="17" x2="12.01" y2="17" />
    </template>
    <template v-else-if="name === 'search'">
      <circle cx="11" cy="11" r="8" />
      <line x1="21" y1="21" x2="16.65" y2="16.65" />
    </template>
    <template v-else-if="name === 'lightbulb'">
      <path d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A6 6 0 0 0 6 8c0 1 .2 2.2 1.5 3.5.7.7 1.3 1.5 1.5 2.5" />
      <line x1="9" y1="18" x2="15" y2="18" />
      <line x1="10" y1="22" x2="14" y2="22" />
    </template>

    <!-- 方向箭头(替换 ↑ ↓ →) -->
    <template v-else-if="name === 'arrow-up'">
      <line x1="12" y1="19" x2="12" y2="5" />
      <polyline points="5 12 12 5 19 12" />
    </template>
    <template v-else-if="name === 'arrow-down'">
      <line x1="12" y1="5" x2="12" y2="19" />
      <polyline points="19 12 12 19 5 12" />
    </template>
    <template v-else-if="name === 'arrow-right'">
      <line x1="5" y1="12" x2="19" y2="12" />
      <polyline points="12 5 19 12 12 19" />
    </template>

    <!-- 导航(sidebar nav 用) -->
    <template v-else-if="name === 'layout-dashboard'">
      <rect x="3" y="3" width="7" height="9" />
      <rect x="14" y="3" width="7" height="5" />
      <rect x="14" y="12" width="7" height="9" />
      <rect x="3" y="16" width="7" height="5" />
    </template>
    <template v-else-if="name === 'funnel'">
      <path d="M22 3H2l8 9.46V19l4 2v-8.54L22 3z" />
    </template>
    <template v-else-if="name === 'line-chart'">
      <path d="M3 3v18h18" />
      <polyline points="7 14 12 9 16 13 21 7" />
    </template>
    <template v-else-if="name === 'user'">
      <path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2" />
      <circle cx="12" cy="7" r="4" />
    </template>
    <template v-else-if="name === 'book-open'">
      <path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z" />
      <path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z" />
    </template>

    <!-- 元素 / 数据 -->
    <template v-else-if="name === 'database'">
      <ellipse cx="12" cy="5" rx="9" ry="3" />
      <path d="M3 5v14a9 3 0 0 0 18 0V5" />
      <path d="M3 12a9 3 0 0 0 18 0" />
    </template>

    <!-- INS-B Phase 1 新加(2026-05-27 末⁵) -->
    <template v-else-if="name === 'bar-chart-3'">
      <!-- 质量观察(B2) -->
      <path d="M3 3v18h18" />
      <path d="M7 16V9" />
      <path d="M12 16V5" />
      <path d="M17 16v-4" />
    </template>
    <template v-else-if="name === 'shield'">
      <!-- 合规风控(B3) -->
      <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" />
    </template>
    <template v-else-if="name === 'compass'">
      <!-- 作者指南针(B7) -->
      <circle cx="12" cy="12" r="10" />
      <polygon points="16.24 7.76 14.12 14.12 7.76 16.24 9.88 9.88 16.24 7.76" />
    </template>
    <template v-else-if="name === 'activity'">
      <!-- 实时事件流(B8) -->
      <polyline points="22 12 18 12 15 21 9 3 6 12 2 12" />
    </template>
    <template v-else-if="name === 'circle-dot'">
      <!-- 在线指示 -->
      <circle cx="12" cy="12" r="10" />
      <circle cx="12" cy="12" r="3" fill="currentColor" />
    </template>

    <!-- BYOK 审核后台(2026-06-05) -->
    <template v-else-if="name === 'eye'">
      <!-- 看截图 -->
      <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" />
      <circle cx="12" cy="12" r="3" />
    </template>
    <template v-else-if="name === 'key'">
      <!-- BYOK 钥匙 -->
      <path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4" />
    </template>

    <!-- INS-B Phase 4 新加(2026-05-27 末⁵³) -->
    <template v-else-if="name === 'users'">
      <!-- 用户群(主导航) -->
      <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" />
      <circle cx="9" cy="7" r="4" />
      <path d="M23 21v-2a4 4 0 0 0-3-3.87" />
      <path d="M16 3.13a4 4 0 0 1 0 7.75" />
    </template>
    <template v-else-if="name === 'chevron-left'">
      <polyline points="15 18 9 12 15 6" />
    </template>
    <template v-else-if="name === 'chevron-right'">
      <polyline points="9 18 15 12 9 6" />
    </template>
    <template v-else-if="name === 'sliders'">
      <!-- 筛选器图标 -->
      <line x1="4" y1="21" x2="4" y2="14" />
      <line x1="4" y1="10" x2="4" y2="3" />
      <line x1="12" y1="21" x2="12" y2="12" />
      <line x1="12" y1="8" x2="12" y2="3" />
      <line x1="20" y1="21" x2="20" y2="16" />
      <line x1="20" y1="12" x2="20" y2="3" />
      <line x1="1" y1="14" x2="7" y2="14" />
      <line x1="9" y1="8" x2="15" y2="8" />
      <line x1="17" y1="16" x2="23" y2="16" />
    </template>
    <template v-else-if="name === 'clock'">
      <circle cx="12" cy="12" r="10" />
      <polyline points="12 6 12 12 16 14" />
    </template>
    <template v-else-if="name === 'arrow-left'">
      <line x1="19" y1="12" x2="5" y2="12" />
      <polyline points="12 19 5 12 12 5" />
    </template>

    <!-- fallback:未注册的 name → 空占位(防 silent failure) -->
    <template v-else>
      <circle cx="12" cy="12" r="1" />
    </template>
  </svg>
</template>

<style scoped>
.icon {
  display: inline-block;
  vertical-align: -0.125rem;
  flex-shrink: 0;
}
</style>

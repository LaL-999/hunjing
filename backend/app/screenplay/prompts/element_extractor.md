...
### 🎯 去小说腔铁律(最高优先级 — 这是"能拍的剧本"与"分镜版小说"的分水岭)

小说和剧本是两种语言:小说写**感受与内心**,剧本写**摄影机拍得到的画面与声音**。
抽 action 时,脑子里想的应是"镜头此刻拍到什么",而不是"读者此刻感受到什么"。
**这条铁律凌驾于"忠于原文用词"之上** —— 原文的散文句子必须被改写成剧本句子,不是照搬。

**四条硬规矩:**
1. **现在时、主动态、短句**。一个 action 一句话一个动作,建议 ≤ 40 字;长了就拆成多个。宁可 3 个短 action,不要 1 个长段落。
2. **零心理词**。删「感到 / 觉得 / 意识到 / 回忆起 / 明白 / 内心 / 暗想 / 仿佛 / 似乎」。这些要么外化成可见动作,要么进 voiceover,**绝不留在 action**。
3. **零上帝旁白**。删作者对人物或命运的评论(如「他不知道,这是他最后一次见到她」)。剧本没有全知叙述者。
4. **零抒情堆砌**。形容词 / 副词砍到只剩必要。「惨白冰冷、死一般寂静的月光」→「月光落在地板上」。

**小说腔 → 剧本腔 对照(照着改):**

✗ 小说腔(原文直搬,读着像散文):
> 夜色渐深,李明感到一种莫名的孤独涌上心头。他缓缓走到窗边,思绪不由自主地飘回了那个遥远的午后。

✓ 剧本腔(拆成可拍的画面,内心戏进 V.O.):
```json
[
  {"type": "action", "text": "李明走到窗边,望着窗外的夜色。"},
  {"type": "voiceover", "character_name": "李明", "text": "又是这样的夜晚。那个下午,好像还没过去多久。", "is_inner_monologue": true}
]
```

✗ 小说腔:
> 她的手微微颤抖着,眼泪在眼眶里打转,却倔强地不肯落下,那是一种混合着愤怒与不甘的复杂情绪。

✓ 剧本腔(情绪由可见细节承载,不写「愤怒与不甘」):
```json
[
  {"type": "action", "text": "她的手在抖。眼眶泛红,泪没落下来。"}
]
```
...
- action.text 单条**建议 ≤ 40 字,硬上限 120 字**(超了说明该拆成多个 action;长段落是小说腔的头号信号)

...
from app.screenplay.services.llm_client import LlmCallFailed, LlmJsonParseFailed, call_json
...
def _persist_bible(
    ...
    try:
        ...
        if old:
            conn.execute("DELETE FROM sp_story_bibles WHERE id = ?", (old["id"],))
            # CASCADE 自动清子表

        bible_id = _new_id()
        conn.execute(
            """INSERT INTO sp_story_bibles (id, novel_id, source, created_at, updated_at)
               VALUES (?, ?, ?, ?, ?)""",
        ...
        for c in characters:
            ...
            aka = c.get("aka") or []
            conn.execute(
                """INSERT INTO sp_bible_characters
        ...
        for l in locations:
            ...
            if int_ext not in ("INT", "EXT", "INT/EXT"):
                int_ext = "INT"
            conn.execute(
                """INSERT INTO sp_bible_locations
        ...
        for r in relationships:
            ...
            rid = uuid.uuid4().hex
            conn.execute(
                """INSERT INTO sp_bible_relationships
    ...
    try:
        ...
        locs = conn.execute(
            """SELECT id, name, int_ext, description
                 FROM sp_bible_locations WHERE bible_id = ? ORDER BY id""",
            (bible_id,),
        ).fetchall()
        rels = conn.execute(

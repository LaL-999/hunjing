...
def test_catalog_resolves_all_categories():
    assert catalog.get_sku("sub_pro_monthly").amount_cents == 13800
    assert catalog.get_sku("sub_super_max_yearly").amount_cents == 1415760
    assert catalog.get_sku("byok_3m").amount_cents == 9000
    assert catalog.get_sku("byok_3m").quantity == 3
    assert catalog.get_sku("credit_medium").amount_cents == 8500
...
def test_catalog_list_has_12_skus():
    assert len(catalog.list_skus()) == 12
...
def test_order_lifecycle_and_fulfillment(make_user):
    ...
    try:
        ...
        assert order["amount_cents"] == 1800

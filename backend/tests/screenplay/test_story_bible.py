...
def test_import_bible_success(client, uploaded_novel):
    ...
    r = client.post(
        f"/api/screenplay/novels/{novel_id}/story-bible",
        json=_VALID_PAYLOAD,
    )
...
def test_import_bible_unknown_novel_returns_404(client):
    r = client.post(
        "/api/screenplay/novels/nonexistent_xxx/story-bible",
        json=_VALID_PAYLOAD,
    )
...
def test_import_bible_overrides_existing(client, uploaded_novel):
    ...
    novel_id = uploaded_novel
    # 第 1 次:2 个角色
    client.post(f"/api/screenplay/novels/{novel_id}/story-bible", json=_VALID_PAYLOAD)
    ...
    r = client.post(f"/api/screenplay/novels/{novel_id}/story-bible", json=new_payload)
    assert r.status_code == 200
    assert r.json()["stats"]["characters"] == 1

    # 查圣经 → 只有新角色
    get_r = client.get(f"/api/screenplay/novels/{novel_id}/story-bible")
...
def test_import_bible_invalid_payload_returns_400(client, uploaded_novel):
    """characters 不是数组 → 400"""
    r = client.post(
        f"/api/screenplay/novels/{uploaded_novel}/story-bible",
...
def test_extract_bible_with_llm_mocked(client, uploaded_novel, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.screenplay.services.story_bible_service.call_json", fake_call_json,
    )

    r = client.post(f"/api/screenplay/novels/{uploaded_novel}/story-bible/auto?max_chapters=3")
    assert r.status_code == 200, r.text
    body = r.json()
...
def test_extract_bible_llm_failure_returns_502(client, uploaded_novel, monkeypatch):
    """LLM 调用炸 → 502"""
    from app.screenplay.services.llm_client import LlmCallFailed
    ...
    r = client.post(f"/api/screenplay/novels/{uploaded_novel}/story-bible/auto")
    assert r.status_code == 502
    assert r.json()["detail"]["code"] == "LLM_EXTRACT_FAILED"


def test_extract_bible_invalid_max_chapters(client, uploaded_novel):
    r = client.post(f"/api/screenplay/novels/{uploaded_novel}/story-bible/auto?max_chapters=0")
    assert r.status_code == 400
...
def test_get_bible_returns_full_structure(client, uploaded_novel):
    novel_id = uploaded_novel
    client.post(f"/api/screenplay/novels/{novel_id}/story-bible", json=_VALID_PAYLOAD)
    r = client.get(f"/api/screenplay/novels/{novel_id}/story-bible")
    assert r.status_code == 200
    body = r.json()
...
def test_get_bible_nonexistent_returns_404(client, uploaded_novel):
    """该 novel 还没建圣经"""
    r = client.get(f"/api/screenplay/novels/{uploaded_novel}/story-bible")
    assert r.status_code == 404
...
def test_aka_resolves_to_same_character(client, uploaded_novel):
    ...
    r = client.post(f"/api/screenplay/novels/{uploaded_novel}/story-bible", json=payload)
    assert r.status_code == 200
    assert r.json()["stats"]["relationships"] == 1   # 别名应被映射,关系生效

    # GET 验证
    get_r = client.get(f"/api/screenplay/novels/{uploaded_novel}/story-bible")

...
def _insert_novel(novel_id: str = "n_test", title: str = "测试小说") -> None:
    """造一个 novel 行(外键依赖 + 阶段 3.5 加 user_id 必填)。

    阶段 7:不再造新 user,而是用 conftest autouse 已建好的「当前测试 user」—
    这样后续 screenplay_store wrapper 的注入 user_id 跟 sp_novels.user_id 一致。
    """
    from app.screenplay.db.connection import get_connection
    conn = get_connection()
    try:
        # 拿 conftest autouse 刚建的 user(reset_test_db 每测试清表,所以只有它一个)
        row = conn.execute("SELECT id FROM users ORDER BY created_at DESC LIMIT 1").fetchone()
        if row is None:
            raise RuntimeError(
                "_insert_novel: 测试库无 user — conftest autouse 应当先建一个"
            )
        user_id = row["id"]
        conn.execute(
...
def test_screenplays_table_exists_after_init(temp_db):
    ...
    try:
        cur = conn.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='sp_screenplays'"
        )
...
def test_screenplays_index_exists(temp_db):
    ...
    try:
        cur = conn.execute(
            "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_sp_screenplays_novel_time'"
        )
        assert cur.fetchone() is not None
    finally:
        conn.close()
...
def test_save_row_in_db(temp_db):
    ...
    try:
        row = conn.execute(
            "SELECT * FROM sp_screenplays WHERE id = ?", (sid,),
        ).fetchone()
...
def test_cascade_delete_when_novel_removed(temp_db):
    ...
    conn = get_connection()
    try:
        conn.execute("DELETE FROM sp_novels WHERE id = ?", ("n1",))
        conn.commit()
    finally:
        conn.close()

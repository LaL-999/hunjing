...
def _mock_llm_returns(monkeypatch, response: dict, usage: dict | None = None):
    ...
    monkeypatch.setattr(
        "app.screenplay.services.pipeline.scene_splitter.call_json", fake,
    )
...
def _get_test_user_id() -> str:
    # 阶段 7:取 conftest autouse 已建的当前测试 user
...
def _upload_novel_with_bible(client) -> tuple[str, str]:
    ...
    upload_resp = client.post(
        "/api/screenplay/novels", files={"file": ("t.txt", io.BytesIO(content), "text/plain")},
    )
    ...
    client.post(f"/api/screenplay/novels/{novel_id}/story-bible", json=bible_payload)
    return novel_id, chapter_id


def test_split_from_db_unknown_novel_raises(client):
    with pytest.raises(ValueError, match="不存在"):
        split_chapter_from_db("nonexistent_xxx", "chapter_xxx", user_id=_get_test_user_id())


def test_split_from_db_chapter_not_in_novel(client):
    novel_id, _ = _upload_novel_with_bible(client)
    with pytest.raises(ValueError, match="不属于"):
        split_chapter_from_db(novel_id, "chapter_999", user_id=_get_test_user_id())
...
def test_split_from_db_without_bible(client, monkeypatch):
    ...
    with pytest.raises(ValueError, match="圣经"):
        split_chapter_from_db(novel_id, chapter_id, user_id=_get_test_user_id())
...
def test_split_from_db_happy_path(client, monkeypatch):
    ...
    result = split_chapter_from_db(novel_id, chapter_id, user_id=_get_test_user_id())
    assert len(result.scenes) == 1
...
def test_endpoint_split_chapter_happy_path(client, monkeypatch):
    ...
    r = client.post(f"/api/screenplay/chapters/{chapter_id}/split")
...
def test_endpoint_chapter_not_found(client):
    r = client.post("/api/screenplay/chapters/nonexistent_xxx/split")
...
def test_endpoint_without_bible_returns_400(client):
    ...
    r2 = client.post(f"/api/screenplay/chapters/{chapter_id}/split")

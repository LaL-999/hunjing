...
def _patch_bible_present(monkeypatch, bible: dict | None = None) -> None:
    ...
    阶段 7:接通父平台后 get_bible(novel_id, user_id=...) 签名变了,
    lambda 用 **kwargs 吞掉所有命名参数。
    """
    real_bible = bible if bible is not None else _mock_bible()
    from app.screenplay.services import story_bible_service
    monkeypatch.setattr(
        story_bible_service, "get_bible",
        lambda nid, **kwargs: real_bible,
    )
...
def _patch_pipeline_happy(monkeypatch) -> None:
    ...
    def fake_split(chapter_input, **kwargs):
    ...
    monkeypatch.setattr(
        ...
        lambda scene_input, **kwargs: _make_extract_result(),
    )
    monkeypatch.setattr(
        compose_service, "refine_attribution",
        lambda st, ch, els, **kwargs: _make_refine_result(els),
    )
    monkeypatch.setattr(
...
def test_scene_splitter_retry_succeeds(temp_db, monkeypatch):
    ...
    attempts: list[int] = []
    def flaky_split(chapter_input, **kwargs):
        attempts.append(1)
        if len(attempts) == 1:
    ...
    monkeypatch.setattr(
        compose_service, "extract_elements", lambda si, **kwargs: _make_extract_result(),
    )
    monkeypatch.setattr(
        compose_service, "refine_attribution", lambda st, ch, els, **kwargs: _make_refine_result(els),
    )
    monkeypatch.setattr(
...
def test_scene_splitter_all_attempts_fail_chapter_skipped(temp_db, monkeypatch):
    ...
    def split_fail_ch1(chapter_input, **kwargs):
...
def test_element_extractor_failure_scene_skipped(temp_db, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda ci, **kwargs: _make_split_result(ci.chapter_number, scene_count=3),
    )

    extract_count: list[int] = []
    def extract_fail_scene2(scene_input, **kwargs):
...
def test_dialogue_attributor_failure_downgrades(temp_db, monkeypatch):
    ...
    def refine_boom(st, ch, els, **kwargs):
...
def test_error_codes_for_known_failures(temp_db, monkeypatch):
    ...
    monkeypatch.setattr(
        compose_service, "split_chapter",
        lambda ci, **kwargs: (_ for _ in ()).throw(SceneSplitError("总是失败")),
    )
    with pytest.raises(compose_service.ComposePipelineError) as ei:

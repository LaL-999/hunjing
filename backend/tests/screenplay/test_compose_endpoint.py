...
from app.screenplay.services import compose_service
from app.screenplay.services.pipeline.adaptation_decision import (
    AdaptationDecision,
    AdaptationOption,
    DecisionResult,
)
from app.screenplay.services.pipeline.dialogue_attributor import RefineResult
...
def _insert_novel_with_chapters(
    ...
    """阶段 7:user_id 取 conftest autouse 已建的「当前测试 user」。"""
    ...
    try:
        ...
        if user_row is None:
            raise RuntimeError("conftest autouse 应已建 user")
        user_id = user_row["id"]
        conn.execute(
            ...
            (novel_id, user_id, chapter_count),
        )
...
def _patch_bible(monkeypatch) -> None:
    ...
    monkeypatch.setattr(story_bible_service, "get_bible", lambda nid, **kwargs: bible)


def _patch_pipeline_happy(monkeypatch) -> None:
    """4 agent 全成功 mock。"""

    def fake_split(ci, **kwargs):
        return SplitResult(
    ...
    def fake_refine(st, ch, els, **kwargs):
...
def test_post_with_default_body(client, monkeypatch):
    ...
    r = client.post("/api/screenplay/novels/n1/compose-screenplay", json={})
    ...
    # 持久化:GET by id 能查回
    r2 = client.get(f"/api/screenplay/screenplays/{body['screenplay_id']}")
...
def test_post_novel_not_found_returns_404(client):
    ...
    r = client.post("/api/screenplay/novels/not-exist/compose-screenplay", json={})
...
def test_post_no_chapters_returns_422(client):
    ...
    r = client.post("/api/screenplay/novels/empty/compose-screenplay", json={})
...
def test_post_all_splits_fail_returns_502(client, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda ci, **kwargs: (_ for _ in ()).throw(SceneSplitError("总失败")),
    )
...
def test_get_latest_after_compose(client, monkeypatch):
    ...
    post_r = client.post("/api/screenplay/novels/n1/compose-screenplay", json={})
    assert post_r.status_code == 200
    sid = post_r.json()["screenplay_id"]

    get_r = client.get("/api/screenplay/novels/n1/screenplay")
...
def test_get_latest_before_compose_returns_404(client):
    ...
    r = client.get("/api/screenplay/novels/n1/screenplay")
    assert r.status_code == 404
...
def test_get_by_id_returns_404_when_missing(client):
    ...
    r = client.get("/api/screenplay/screenplays/does-not-exist")
    assert r.status_code == 404
    assert r.json()["detail"]["code"] == "SCREENPLAY_NOT_FOUND"
